...
const updateUsuario=async(id,img)=>{
    await Usuario.findByIdAndUpdate(id,{'foto_usuario': img})
    const userUpdated = await Usuario.findById(id).populate('rol').populate({ path: 'archivos',
    populate: {
        path: 'asignatura'
    }
});
    return await buildJWT(userUpdated)
}
